# Trend Micro Unified Download Tool (v2.2.1)
# Desteklenen Ürünler: Apex One, Apex Central, Deep Security Manager (LTS), DDAN, DDD, DDI, DDEI
# Yazar: bab-ı kod
# ---------------------------------------------------------------------------

# === EXECUTION POLICY BYPASS & RESTORATION ===
...
try {
    ...
    $global:ScrapingProducts = @{
        "1" = @{ Name = "Apex One"; ProdId = "1745" }
        "2" = @{ Name = "Apex Central"; ProdId = "1746" }
        "3" = @{ Name = "Deep Discovery Analyzer"; ProdId = "1646"; UpgradePath = "https://success.trendmicro.com/en-US/solution/KA-0010446" }
        "4" = @{ Name = "Deep Discovery Director"; ProdId = "1720"; UpgradePath = "https://success.trendmicro.com/en-US/solution/KA-0010445" }
        "5" = @{ Name = "Deep Discovery Inspector"; ProdId = "1663"; UpgradePath = "https://success.trendmicro.com/en-US/solution/KA-0010444" }
        "6" = @{ Name = "Deep Discovery Email Inspector"; ProdId = "1666"; UpgradePath = "https://success.trendmicro.com/en-US/solution/KA-0010447" }
    }
    ...
    function Wait-ForKeyPress {
        ...
        Write-Color "`n$Message" -Color $COLOR_GUMUS
        try {
            $null = $Host.UI.RawUI.ReadKey("NoEcho,IncludeKeyDown")
        }
        catch {
            Write-Host "(Enter tusuna basin)" -ForegroundColor $COLOR_GUMUS
            $null = Read-Host
        }
    }
    ...
    function Clear-AppCache {
        ...
        if (Test-Path $cacheDir) {
            ...
            Write-Color "Temizlenecek önbellek bulunamadı." -Color $COLOR_GUMUS
        }
    }
    ...
    function Get-TMPackageInfo {
        ...
        if ($Type -eq "Main") {
            if ($Text -match '(?i)(?:Filename:\s*)?(\S+\.(?:exe|msi|zip|bin|iso))') { $res.Filename = $matches[1] }
    ...
    function Get-ProductInfo {
        ...
        try {
            ...
            $sectionMap = @{ "Main" = 0; "Update" = 1; "Hotfix" = 2 }
            ...
            if (-not $Silent) {
                ...
                if ($Product.UpgradePath) {
                    Write-Color "`n[!] SOFTWARE UPGRADE PATH KB:" -Color $COLOR_YAKUT
                    Write-Color "    $($Product.UpgradePath)" -Color $COLOR_ALTIN
                }

                foreach ($type in @("Main", "Update", "Hotfix")) {
                    $p = $results[$type]
                    ...
                    Write-Color "  Link: $($p.DownloadLink)" -Color $COLOR_GUMUS
                }
            }
            return $results
        }
    ...
    function Get-DeepSecurityManagerInfo {
        ...
        Write-Color "`nEnvanter dosyası çekiliyor (LTS Sürümleri): $xmlUrl" -Color $COLOR_GUMUS
    ...
    function Confirm-FileOverwrite {
        ...
        if (Test-Path $FilePath) {
            ...
            Write-Color "Boyut: $([math]::Round($f.Length/1MB,2)) MB - Tarih: $($f.LastWriteTime)" -Color $COLOR_GUMUS
    ...
    function Start-BackgroundDownload {
        ...
        Write-Color "Takip için ana menüden 'Durum Takibi' sekmesini kullanabilirsiniz." -Color $COLOR_GUMUS
    }

    function Show-Status {
        if ($global:ActiveDownloads.Count -eq 0) { Write-Color "Aktif indirme yok." -Color $COLOR_GUMUS; return }
    ...
    function Start-MainMenu {
        while ($true) {
            Show-MebadiBanner
            Write-Host "`n[ Trend Micro Unified Download Tool - v2.2.0 ]" -ForegroundColor $COLOR_ALTIN
            Write-Host " 1. Product Packets (Apex / Deep Discovery)"
            ...
            switch ($mChoice) {
                "1" {
                    Write-Header "PRODUCT SELECTION"
                    Write-Color " 1. Apex One"
                    Write-Color " 2. Apex Central"
                    Write-Color " 3. Deep Discovery Analyzer (DDAN)"
                    Write-Color " 4. Deep Discovery Director (DDD)"
                    Write-Color " 5. Deep Discovery Inspector (DDI)"
                    Write-Color " 6. Deep Discovery Email Inspector (DDEI)"
                    Write-Color " B. Back"
                    $selection = Read-Host "`nProduct Number"
                    if ($selection -match "[1-6]") {
                        $prod = $global:ScrapingProducts[$selection]
                        $infos = Get-ProductInfo -Product $prod
                        if ($null -ne $infos) {
                            $ask = Read-Host "`nStart download? [E/H]"
                            if ($ask -eq "E" -or $ask -eq "e") {
                                $folder = Select-Folder -Prod $prod.Name
                                Show-MebadiBanner
                                Write-Header "PACKAGE SELECTION"
                                Write-Color " 1. Main Package (Full/ISO)"
                                Write-Color " 2. Update Package"
                                Write-Color " 3. Yama/Hotfix"
                                Write-Color " 4. All of them"
                                $target = Read-Host "`nPackage to download"
                                if ($target -in "1", "4") { 
                                    if ($infos.Main.DownloadLink) {
                                        Start-BackgroundDownload -Url $infos.Main.DownloadLink -OutputPath (Join-Path $folder $infos.Main.Filename) -Description "Main Package" -ProductName $prod.Name 
                                    }
                                }
                                if ($target -in "2", "4") {
                                    if ($infos.Update.DownloadLink) {
                                        Start-BackgroundDownload -Url $infos.Update.DownloadLink -OutputPath (Join-Path $folder $infos.Update.Filename) -Description "Update Package" -ProductName $prod.Name 
                                    }
                                }
                                if ($target -in "3", "4") { 
                                    if ($infos.Hotfix.DownloadLink) {
                                        Start-BackgroundDownload -Url $infos.Hotfix.DownloadLink -OutputPath (Join-Path $folder $infos.Hotfix.Filename) -Description "Hotfix Package" -ProductName $prod.Name 
                                    }
                                }
                                Wait-ForKeyPress
                            }
                    ...
                    if ($null -ne $platform) {
                        ...
                        if ($null -ne $list) {
                            ...
                            for ($i = 0; $i -lt $list.Count; $i++) {
                                ...
                                Write-Host ("   SHA256: $($item.SHA256)") -ForegroundColor $COLOR_GUMUS
                                Write-Host ("   MD5:    $($item.MD5)") -ForegroundColor $COLOR_GUMUS
                                Write-Host ("   Tarih:  $($item.Updated)") -ForegroundColor $COLOR_GUMUS
                ...
                "4" {
                    ...
                    if ($global:DownloadHistory.Count -eq 0) { Write-Color "Gecmis kaydi bulunamadi." -Color $COLOR_GUMUS }
    ...
    Write-Host "`n*Gayret bizden, tevfik Allah'tandir. | bab-ı kod (v2.2.1)*"
}
