...
# === EXECUTION POLICY BYPASS UYGULAMA ===
try {
    # Mevcut policy'yi kaydet
    $currentPolicy = Get-ExecutionPolicy -Scope Process
    
    # Process scope için Bypass ayarla (sadece bu oturum için)
    Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass -Force
    
    Write-Host "✓ Execution Policy geçici olarak Bypass edildi (sadece bu script için)." -ForegroundColor Green
    Write-Host "  Önceki Policy: $currentPolicy" -ForegroundColor Gray
    Write-Host ""
    
    # Script bloğunu çalıştır
    & $ScriptBlock @PSBoundParameters
}
catch {
    Write-Host "HATA: Script çalıştırılırken kritik bir hata oluştu:" -ForegroundColor Red
